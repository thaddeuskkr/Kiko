...
module.exports = async (client, message) => {
  ...
  if (
    message.content === 'Kiko deploy guild' ||
    message.content === 'Kiko deploy' ||
    message.content === 'Kiko deploy global' ||
    message.content === 'Kiko undeploy global' ||
    message.content === 'Kiko undeploy' ||
    message.content === 'Kiko undeploy guild'
  ) {
    const commands = []
    ...
    if (message.content === 'Kiko deploy guild') {
      rest
        ...
        .then(() =>
          client.logger.info(
            `Successfully registered application commands for server ${message.guild.name} (${message.guild.id})!`,
          ),
        )
        .catch((err) =>
          client.logger.error(
            'Failed to register application commands: ' + err.message,
          ),
        )
      ...
      rest
        .get(Routes.applicationGuildCommands(client.user.id, message.guild.id))
        .then((data) => {
          const promises = []
          ...
          return Promise.all(promises).then(() =>
            client.logger.info(
              `Successfully unregistered application commands for server ${message.guild.name} (${message.guild.id})!`,
            ),
          )
        })
      return message.reply('Success!')
      ...
      rest.get(Routes.applicationCommands(client.user.id)).then((data) => {
        ...
        return Promise.all(promises).then(() =>
          client.logger.info(
            'Successfully unregistered application commands globally!',
          ),
        )
      })
      return message.reply('Success!')
  ...
  if (
    message.channel.id === client.config.evalChannel &&
