...
const keyv = new Keyv(process.env.KEYV, { namespace: 'Kiko' })
...
const categories = categoriesDirents
    ...
    if (!cmd.data?.name) {
      client.logger.warn(
        `Command ${command.replace(
          '.js',
          '',
        )} has insufficient data. Skipping...`,
      )
      break
    }
...
client.logger.info(
  `Loaded ${commandCount} commands in ${categories.length} categories!`,
)
...
process.on('unhandledRejection', async (reason, promise) => {
  ...
  for (const ownerId of client.config.owners) {
    await client.users.cache
      .get(ownerId)
      .send(`**Unhandled rejection at ${promise}:**\n\`\`\`${reason}\`\`\``)
  }
})
